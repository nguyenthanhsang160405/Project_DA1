<?php
//isset($data) && !empty($data): Kiểm tra xem biến $data có tồn tại và không rỗng không.
//Nếu có, tiếp tục kiểm tra các phần tử trong $data
if(isset($data) && !empty($data)){
    //$data['notification_pass']: Nếu tồn tại và không rỗng, gán vào biến
    if(isset($data['notification_pass']) && !empty($data['notification_pass'])){
        //$notification_pass. Đây là thông báo liên quan đến việc thay đổi mật khẩu.
    ...
    //Nếu có lỗi nào liên quan đến thông tin người dùng
    //(như tên, số điện thoại, địa chỉ...), nó sẽ được gán vào biến $err (chứa các lỗi cần hiển thị).
    ...
    <div class="column2">
        <form action="index.php?page=usermanage" method="post">
            ...
            <div class="form-group">
                ...
                <input name="pass" type="password" value="<?php if(isset($ifm['pass'])&& !empty($ifm['pass'])) echo $ifm['pass']?>" id="password" placeholder="">
            </div>
            <div class="form-group">
                <label for="confirm-password">Nhập lại mật khẩu: </label><span><?php if(isset($err['xnpass'])&& !empty($err['xnpass'])) echo $err['xnpass']?></span><br>
                <input name="xnpass" value="<?php if(isset($ifm['xnpass'])&& !empty($ifm['xnpass'])) echo $ifm['xnpass']?>" type="password" id="confirm-password" placeholder="">
            </div>
